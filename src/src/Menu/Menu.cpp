...
U8G2_SSD1306_128X64_NONAME_F_HW_I2C u8g2(U8G2_R0, /* reset=*/ U8X8_PIN_NONE, /* clock=*/ CLOCK_PIN, /* data=*/
                                         DATA_PIN);
...
MenuItem *MenuItem::getItem(const char *thisName) {
    ...
    workingItem = this;
    if (strcmp(itemName, thisName))
    // This menu is NOT the item we're looking for, so look through it's children and subsequent peers
    {
        if (workingItem->childItem)
        // if this item has a child, go into this child to check if it is the one we're looking for or if it's one of it's descendents ...
        {
        ...
        while (workingItem) // Now we're looking through the list of subsequent menu items to check
        {
            if (strcmp(workingItem->itemName, thisName) == 0)
                return workingItem;
            if (workingItem->childItem)
            // there's another child menu, so will need to check it and it's descendents for the right one ...
            {
                workingItem = workingItem->childItem->getItem(thisName);
                if (workingItem)
                    return workingItem;
...
MenuItem::MenuItem( std::function<void(const char *, int)> _callback,const char *thisName, MenuItem *thisParent, MenuItem *thisChild, MenuItem *thisPrev,
                   MenuItem *thisNext, int thisMin, int thisMax, int thisDefault, int thisInc) : callback(std::move(_callback)) {
...
int MenuItem::addItem(const char *menuName, const char *thisName, int minVal, int maxVal, int defaultVal,
                      int increment) {
    MenuItem *previous;
    MenuItem *workingItem;
    ...
    if (strcmp(itemName, menuName))
    // This menu is NOT the parent menu we're looking to insert an item into, so try finding the right menu in its descendents or subsequent items ...
    {
        if (childItem)
            // if this item has a child, go into this child to check if it is the correct 'parent' and to add item to it or its descendents ...
        ...
        workingItem = nextItem;
        while (workingItem)
        // Now we're looking through the list of subsequent menu items to check if these are the ones we need to add the item to:
        {
            if (strcmp(workingItem->itemName, menuName) == 0)
                break;
            // we've found it - drop through and process adding the new item as don't need to check any more children or subsequent items ...
            if (workingItem->childItem)
                // there's another child menu, so will need to check it and it's descendents for the right parent menu ...
    ...
    MenuItem *newMenuItem = new MenuItem(callback,thisName, workingItem,NULL, previous,NULL, minVal, maxVal, defaultVal,
                                         increment);
...
void updateMenu() // This draws the menu
{
    ...
    {
        if (currentMenuItem->prevItem != NULL)
        // if there's a previous menu item, draw it in small font above the selected one
        {
        ...
        u8g2.print(buffer);
        if (currentMenuItem->nextItem != NULL)
        // if there's a subsequent menu item, draw it as well in smaller font below the selected one ...
        {
            u8g2.setFont(fontSmall);
...
void menuService() // must be called regularly to check the encoder, process the menu system, and update the LCD
{
    ...
    if ((digitalRead(ENC_SW) == LOW) && (millis() >= (lastPress + 300))) {
        ...
        {
            changingParams = 0;
            topMenu->callback(currentMenuItem->itemName, currentMenuItem->defaultVar);
